...
- Two Families ship: `justerm`, and `justpdf` including the `justbig2` and `justjp2` codecs it consumes. Those two were the borderline call — they are usable on their own and could have stayed independently discoverable rows — and they went in, because a visitor meeting them cold learns more from "part of justpdf" than from a codec name. `justpdf` is also a Family named after its flagship Package, so `/open-source/justpdf` lists a member called `justpdf`. That is normal, not a bug to fix: the Family row and the member row mean different things and only the Family row appears in the list.
